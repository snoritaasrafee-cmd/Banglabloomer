// NASA API key
const apiKey = 'G8Gas1lsJXauEbFuavZraSr6ivd0awV9g13zGrxU';
...
async function showData(type, title){
  ...
  if(type==='Wildflowers'){
    // Live NASA GLOBE Observer API fetch
    const globeURL = `https://api.globe.gov/observations?protocol=LandCover&phenomenon=Wildflower&startdate=2025-03-01&enddate=2025-06-01&latitude=${latitude}&longitude=${longitude}&geojson=true&api_key=${apiKey}`;
    try{
      ...
      console.error(err);
      alert("Could not fetch GLOBE data. Using example data.");
...
async function loadWildflowers(){
  ...
  try{
    ...
    console.error(err);
    alert("Could not fetch wildflower data. Showing example points.");
    for(let i=0;i<7;i++){
